## This script has been created for AWS and involves saving files on s3, use local_passive_learning_pretrained_partial_training_option.py to run on local
import torch
...
# Function to set all seeds for reproducible runs
...
# Fit a loader to load the data in batches; num_workers set to 0 for reproducibility
...
## This code is commented out but basically does training on the entire train set and evaluates on the test set and prints the Dice score
# model = smp.Unet(
...
# You can choose to see or save images generated by the model but uncommenting this requires previous code to be uncommented as well as it requires the model to make predictions
# for test in test_ds:
#     img, mask, fname = test
...
def evaluate_model_on_subset(dataset, subset_indices, test_loader, epochs=5, warm_model=None, seed = 0):
    ...
    set_all_seeds(seed)
    # Retrain or partial train based on iteration and mode
...
# Passive Learning Loop parameters
initial_size = 100
increment = 200
...
# Passive Learning Training Loop
for sim in range(n_simulations):
    ...
    for i, size in enumerate(dataset_sizes):
        ...
        if reset_model:
            ...
            # DEBUG: To check if seeds are working as expected and producing reproducible output
...
# Plotting; we don't show the plots in this case as typically the use case of this script was to run remotely and avoid visualization while the script was running
...
# You can choose to save the entire model if you do run the model training on the complete dataset
...
# Ensure EC2 instance doesn't keep running after script is done running
