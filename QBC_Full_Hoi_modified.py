...
for sim in range(n_simulations):
    ...
    labeled_indices = shuffled_indices[:initial_size]
    unlabeled_indices = shuffled_indices[initial_size:]
    warm_model = None

    while len(labeled_indices) <= max_size:
        size = len(labeled_indices)
        reset_model = USE_WARM_START and RESET_EVERY_N > 0 and len(train_results) % RESET_EVERY_N == 0
        current_subset = labeled_indices if reset_model else labeled_indices[-query_size:]
