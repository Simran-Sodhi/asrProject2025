...
def evaluate_model_on_subset(dataset, subset_indices, test_loader, epochs=10):
...
plt.grid(True)
plt.savefig("PassiveLearningMeanTrainingDiceScore.png", bbox_inches='tight')
print("Saved Figure")
plt.show()
...
plt.ylabel("Mean Test Set Dice Score")
plt.grid(True)
plt.savefig("PassiveLearningMeanTestDiceScore.png", bbox_inches='tight')
print("Saved Figure")
plt.show()

train_df = pd.DataFrame(train_results)
train_df.to_csv("PassiveLearningTrainDiceScores.csv", index=False)

test_df = pd.DataFrame(test_results)
test_df.to_csv("PassiveLearningTestDiceScores.csv", index=False)
